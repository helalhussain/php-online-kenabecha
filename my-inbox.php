      ...
      <!-----------------list-header---------------------->
				...
				<?php
					...
					while($in_res =mysqli_fetch_assoc($in_sql)){
						...
						?>
						<?php
						if($result['user_id'] == $mess['seller_id']){
							$seller_id = $mess['user_id'];
						 $sender_query= "SELECT * FROM user WHERE user_id='$seller_id'";
						 $sender_data = mysqli_query($conn,$sender_query);
						 while($sender = mysqli_fetch_assoc($sender_data)){
				...
				<li class="list-group-item d-flex justify-content-between align-items-center"style="">
                        <div class="name"> <span class="badge badge-info badge-pill"><h6 class="word-spacing:2px; p-1"> <?php echo $sender['user_email']; ?></h6></span></div>
                     <div class="delete">
                       </div> 
                     ...
                     <?php echo "<a class='btn btn-primary' href='my-inbox-show.php?m_id=$mess[m_id]'>View</i></a>";?>
                     <?php echo "<a class='btn-btn-info' href='delete-message.php?m_id=$mess[m_id]' onclick='return checkdelete()'><i class='fa fa-trash fa-lg text-danger'></i></a>";?>
